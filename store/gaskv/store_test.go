...
func TestGasKVStoreBasic(t *testing.T) {
	...
	st.Delete(keyFmt(1))
	require.Empty(t, st.Get(keyFmt(1)), "Expected `key1` to be empty")
	require.Equal(t, meter.GasConsumed(), types.Gas(766))
}
...
func TestGasKVStoreIterator(t *testing.T) {
	...
	iterator.Next()
	require.Equal(t, types.Gas(1680), meter.GasConsumed())
	kc := iterator.Key()
	require.Equal(t, kc, keyFmt(3))
	vc := iterator.Value()
	require.Equal(t, vc, valFmt(0))
	iterator.Next()
	require.Equal(t, types.Gas(1709), meter.GasConsumed())
	require.False(t, iterator.Valid())
	require.Panics(t, iterator.Next)
	require.Equal(t, types.Gas(1714), meter.GasConsumed())
	require.NoError(t, iterator.Error())
	...
	require.Equal(t, types.Gas(1835), meter.GasConsumed())
}
